# Katakana duvar kagidini ikinci monitorde acar.
#
# HIRAGANA SURUMUNDEN FARKI: ayar yok, dolayisiyla tercih sunucusu da yok.
# Hiragana surumu ayarlari prefs.json'a yazmak icin node ile kucuk bir sunucu
# baslatiyordu; burada saklanacak bir tercih olmadigi icin dosya dogrudan
# file:// ile aciliyor. Node kurulu olmasa da calisir.
#
# Parametreler:
...
$page = Join-Path $PSScriptRoot 'katakana\index.html'
...
$lnk = Join-Path $startupDir 'Dilhane Katakana duvar kagidi.lnk'
...
if ($Startup) {
  ...
  $sc.WindowStyle = 7
  $sc.Description = 'Dilhane katakana canli duvar kagidi'
  $sc.Save()
...
if (-not (Test-Path $page)) {
  ...
  Write-Host "  Once calistir:  npm run gen:katakana-wallpaper"
  exit 1
}
...
# Hiragana duvar kagidiyla ayni anda calisabilsin diye profil de ayri.
$profile = Join-Path $env:LOCALAPPDATA 'DilhaneWallpaperKatakana'
